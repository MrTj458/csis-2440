    ...
    <head>
        ...
        <script>
          function newAccount() {
            ...
            const letterName = /^[a-zA-Z]+$/
            const nameInput = document.getElementById("CapName")
            if (nameInput.value == "" || !nameInput.value.match(letterName)) {
              nameInput.classList.add("is-invalid");
              error = true;
              addError("Invalid name entered")
            }

            const emailExpression = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
            const emailInput = document.getElementById("Email")
            if (emailInput.value === "" || !emailExpression.test(emailInput.value)) {
            ...
            if (passInput.value === "" || !passInput.value.match(passwordExpression)) {
              passInput.classList.add("is-invalid")
              error = true
              addError("Invalid password entered")
            }
          ...
          function addError(text) {
            const errors = document.getElementById("errorlog")
...
<div class="form-group">
  ...
  <div class="col-md-4">
  <input id="Submit" name="Submit" type="Submit" placeholder="" class="form-control input-md" onclick="return newAccount()">
